...
class SvDownloader{
  constructor(ffid){
    this.url = `https://forums.sufficientvelocity.com/threads/${ffid}/reader/`;
  }
  ...
  async collect_info(){
    ...
    let title_element = doc.querySelector("div.p-title h1.p-title-value")

    // remove tag text
    let title_text = Array.from(title_element.childNodes).map((node) => {
    ...
    this.info = "";
    this.summary = "";
    this.epub = new Epub(this.author, this.title, this.summary, this.url, this.info, "sv");
